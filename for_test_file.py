...
# Начальные настройки
folder_name = r'd:\Clouds\Google Диск\Programming\Python\Поколение Python. Курс для проФессионалов\zip'  # Папка с расположением zip-архивов
# file_zip = get_latest_file(folder_name)  # Автоматически получить имя файла, у которого самая свежая дата модификации
...
def hide_card(card_number):
    return ''.join(['*' if i < 12 else x for i, x in enumerate(card_number.replace('',''))])
...
with ZipFile(f'{folder_name}/{file_zip}', 'r') as zip_file:
    ...
    for i in range(1, t_numbers + 1):
        ...
        with zip_file.open(f'{i}', 'r') as t, \
            ...
            if answer == a: kol_right += 1
            print()
    print(' Итого тестов:'.ljust(18), '\033[1;3;34m', t_numbers, '\033[m')
    print(' Пройдено тестов:'.ljust(18), '\033[1;3;32m' if t_numbers == kol_right else '\033[31m', kol_right, '\033[m')